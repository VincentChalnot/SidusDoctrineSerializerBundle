...
class EntityDenormalizer implements DenormalizerInterface
{
    ...
    protected function findPersistedInstance(string $class, $data)
    {
        ...
        $mappings = array_merge($classMetadata->associationMappings, $classMetadata->fieldMappings);
        foreach ($mappings as $name => $mapping) {
            if ($mapping['unique'] ?? false) {
                $result = $this->findBy($entityManager, $class, [$name], $data);
                if ($result) {
                    return $result;
                }
            }
